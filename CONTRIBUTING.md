...
`isbnlib-porbase` has a very small code base, so it is a good project to begin your
...
7. Pass **all tests** and with **coverage > 90%**.
   ...
   `pytest -q --cov=isbnlib_porbase --cov-fail-under=90 isbnlib_porbase/test/test*.py`.
